...
** TODO make the skill activation position tables on the right scroll when they run out of room instead of causing the viewport to scroll
...
* bugs
** TODO shorter skills can overlap longer skills if the shorter skill is placed first
need to check if b.start/end is within in r in addition to r within b
** TODO skill id is repeated on each alternative
** TODO tab order in expanded uma pane is wrong
** TODO expanding uma pane sets selected uma to 2
